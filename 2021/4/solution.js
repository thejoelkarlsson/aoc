...
class Row {
 ...
 drawNumber(number) {
  ...
  this.hasBingo =
   this.row.filter((number) => number.isDrawn).length === this.row.length;
 }
...
class Column {
 ...
 drawNumber(number) {
  ...
  this.hasBingo =
   this.column.filter((number) => number.isDrawn).length === this.column.length;
 }
}
...
class BingoBoard {
 ...
 drawNumber(number) {
  [...this.rows, ...this.columns].forEach((rowOrColumn) =>
   rowOrColumn.drawNumber(number)
  );

  this.hasBingo =
   [...this.rows, ...this.columns].some((rowOrColumn) => rowOrColumn.hasBingo) >
   0;

  this.score =
   [...this.rows.map((row) => row.getScore())].reduce(
    (prev, curr) => prev + curr
   ) * +number;
 }
}
